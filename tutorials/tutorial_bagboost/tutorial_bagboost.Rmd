...
```{r bagging, exercise=TRUE, exercise.eval=FALSE}
set.seed(333)
...
```{r rf, exercise=TRUE, exercise.eval=FALSE}
...
To do so, we create a separate dataframe called `dataset_edited` in which we mutate our target variable to a conformable form (Note that copying the entire dataset is not the most efficient way of doing it. However, since we are dealing witha very small dataset and want to avoid confusion, we create a separate copy.)
...
Boosting comes with much more parameters to choose than any previous tree-based modesl:
...
* `interaction.depth` -- specifies the maximum depth of tree. Remember that in boosting we can even grow trees with depth 1, i.e. assiging a very small task to each "ant", but the true interaction depth comes from the combination of many "ants"  working sequentially like in a relay race.
...
quiz(
  question("From the plot you can see that:",
    ...
    answer("The out-of-sample fit first deteriorates and then improves with the number trees becuase of overfitting"),
    answer("The out-of-sample fit first improves and then deteriorates with the number trees becuase of overfitting", correct = TRUE)
  )
)
```
