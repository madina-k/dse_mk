...
train <- sample(x = c(_, _), size = ____, replace = ____, prob = c(_, _))
...
```{r quiz_whichvars}
quiz(
  question("Which variables does the tree use to predict  recidivism?",
...
We choose `type = "class"` becasue we are interested in the prediction of the class (not in the vector of probabilities over different classes).
...
```{r tree_predict_exec, echo=FALSE}
set.seed(456)
pruned_predict <- predict(object = prune_compas, 
                        newdata = dataset, 
                        type = "class")
...
## 3) Misclassification errors and fairness
